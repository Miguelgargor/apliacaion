...
def main():
    ...
    color_especial_arista = st.color_picker('**Color especial arista**', value='#00FFAA', format='')
    color_normal_arista = st.color_picker('**Color normal arista**', value='#00FFAA', format='')
    tamaño_letra = st.slider('**Tamaño Fuente**', min_value=1, max_value=20, value=8, step=1, format='') # Step is integer
    width = st.slider('**Ancho figura**', min_value=1.0, max_value=16.0, value=1.0, step=0.1, format='') # Step is float
    height = st.slider('**Alto figura**', min_value=1.0, max_value=16.0, value=1.0, step=0.1, format='') # Step is float
